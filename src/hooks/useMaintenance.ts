...
import type { MaintenanceRecord, MaintenanceAlert } from '../types';
...
export const useMaintenance = () => {
  ...
  const getAlerts = (): MaintenanceAlert[] => {
    return MAINTENANCE_RULES.map(rule => {
      ...
      if (!lastRecord) {
        // Forzamos el tipo explícitamente para evitar error de string
        const unknownStatus: 'unknown' = 'unknown';
        return { 
          rule, 
      ...
      // Definimos la variable con el tipo estricto
      let status: 'danger' | 'warning' | 'ok' = 'ok';
      
      if (isOverdue) status = 'danger';
      else if (isClose) status = 'warning';
